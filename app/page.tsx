...
function ScrollReveal({
  ...
  const translateX = useTransform(scrollYProgress, [0, 0.5], [direction === "left" ? -distance : distance, 0]);
  const translateY = useTransform(scrollYProgress, [0, 0.5], [distance, 0]);
  const motionStyle = direction === "left" || direction === "right" ? { opacity, x: translateX } : { opacity, y: translateY };
  return (
...
const PROJECT_STATIC = [
  { gradient: "from-violet-500/10 to-purple-500/10", border: "hover:border-violet-500/40", statusKey: "completed" as const, github: "https://github.com/Juaker1/Proyecto-toxinas", demo: null as string | null, images: ["/images/projects/bioinformatica.png"] },
  { gradient: "from-purple-500/10 to-fuchsia-500/10", border: "hover:border-purple-500/40", statusKey: "completed" as const, github: "https://github.com/Juaker1/DocuMind", demo: null as string | null, images: ["/images/projects/documind.png"] },
  { gradient: "from-fuchsia-500/10 to-pink-500/10", border: "hover:border-fuchsia-500/40", statusKey: "completed" as const, github: "https://github.com/Juaker1/Medicion-Imagenes", demo: null as string | null, images: ["/images/projects/micromeasure1.jpg", "/images/projects/micromeasure2.jpg"] },
  { gradient: "from-violet-500/10 to-blue-500/10", border: "hover:border-violet-500/40", statusKey: "completed" as const, github: "https://github.com/Juaker1/task-manager", demo: null as string | null, images: ["/images/projects/taskmanager.png"] },
];
...
function ContactCard({ opt }: { opt: ContactOpt }) {
    ...
    <>
      ...
      {opt.copyable && (
        <button
          ...
          aria-label={opt.copyAriaLabel ?? "Copy"}
  ...
  return (
    <ScrollReveal direction={opt.dir} className="h-full relative">
      ...
      <div
        className={`absolute -top-10 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-zinc-800 border border-violet-500/30 text-violet-400 text-xs font-mono rounded-full whitespace-nowrap pointer-events-none transition-all duration-300 ${
...
export default function Home() {
  ...
  const projects = PROJECT_STATIC.map((meta, i) => ({
    ...meta,
    ...T.projects.items[i],
    status: T.projects.status[meta.statusKey],
  }));

  const contactCards: ContactOpt[] = [
    {
      label: T.contact.cards[0].label,
      value: "joaquin.aguilar.ampuero@gmail.com",
      href: "mailto:joaquin.aguilar.ampuero@gmail.com",
      desc: T.contact.cards[0].desc,
      bgIcon: "bg-violet-500/10", textIcon: "text-violet-400",
      border: "border-violet-500/20 hover:border-violet-500/50",
      icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
      dir: "left",
      copyable: true,
      toastMsg: T.contact.emailCopied,
      copyAriaLabel: T.contact.copyAriaLabel,
    },
    {
      label: T.contact.cards[1].label,
      value: "linkedin.com/in/joaquin-aguilar-ampuero",
      href: "https://linkedin.com/in/joaquin-aguilar-ampuero",
      desc: T.contact.cards[1].desc,
    ...
    {
      label: T.contact.cards[2].label,
      value: "github.com/Juaker1",
      href: "https://github.com/Juaker1",
      desc: T.contact.cards[2].desc,
    ...
    {
      label: T.contact.cards[3].label,
      value: "Open to work",
      href: "mailto:joaquin.aguilar.ampuero@gmail.com",
      desc: T.contact.cards[3].desc,
      bgIcon: "bg-violet-500/10", textIcon: "text-violet-400",
      border: "border-violet-500/20 hover:border-violet-500/50",
    ...
    <CardDeck sections={[
      ...
      <section id="inicio" key="inicio" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute inset-0 opacity-[0.06]" style={{ backgroundImage: "radial-gradient(circle, #8b5cf6 1px, transparent 1px)", backgroundSize: "36px 36px" }} />
          <div className="absolute inset-0 opacity-[0.025]" style={{ backgroundImage: "repeating-linear-gradient(45deg, #8b5cf6 0px, #8b5cf6 1px, transparent 0px, transparent 50%)", backgroundSize: "24px 24px" }} />
        </div>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 flex flex-col-reverse lg:flex-row items-center justify-between gap-16">
          ...
          <ScrollReveal direction="left" className="flex-1 text-center lg:text-left">
            ...
            <h2 className="text-xl lg:text-3xl font-semibold text-zinc-400 mb-6">
              {T.hero.subtitle1}{" "}
              <span className="text-violet-400">{T.hero.subtitle2}</span>
              <span className="text-zinc-600"> {T.hero.subtitleDot}</span>
            </h2>
      ...
      <section id="sobre-mi" key="sobre-mi" className="border-t border-zinc-800/50 py-24">
        <div className="max-w-6xl mx-auto px-6">
          <ScrollReveal direction="up">
            <span className="text-violet-400 font-mono text-sm">{T.about.comment}</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-white mt-2 mb-12">{T.about.heading}</h2>
          </ScrollReveal>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            <ScrollReveal direction="left" className="flex-shrink-0 w-full lg:w-64">
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 space-y-3">
                <h3 className="text-xs font-mono font-semibold text-violet-400 uppercase tracking-wider mb-3">{T.about.quickInfoTitle}</h3>
                {T.about.infoItems.map(({ icon, label, value }) => (
              ...
              <a
                ...
                className="mt-3 flex items-center justify-center gap-2 px-4 py-3 bg-violet-600 hover:bg-violet-500 text-white font-bold rounded-lg transition-all text-sm"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <ScrollReveal direction="right" className="flex-1">
              <h3 className="text-xl font-bold text-white mb-5">{T.about.storyTitle}</h3>
              <div className="space-y-4 text-zinc-400 leading-relaxed mb-10">
                {T.about.story.map((p, i) => <p key={i}>{p}</p>)}
              </div>
              <div>
                <span className="text-violet-400 font-mono text-sm">{T.about.educationComment}</span>
                <h4 className="text-lg font-bold text-white mt-1 mb-5">{T.about.educationTitle}</h4>
      ...
      <section id="stack" key="stack" className="border-t border-zinc-800/50 bg-zinc-900/30 py-24">
        <div className="max-w-6xl mx-auto px-6">
          <ScrollReveal direction="up" className="mb-12 text-center">
            <span className="text-violet-400 font-mono text-sm">{T.stack.comment}</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-white mt-2">{T.stack.heading}</h2>
          </ScrollReveal>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
            {T.stack.techGroups.map((group, i) => (
          ...
          <ScrollReveal direction="up" className="border-t border-zinc-800 pt-10">
            <span className="text-violet-400 font-mono text-sm">{T.stack.learningComment}</span>
            <h3 className="text-lg font-bold text-white mt-1 mb-5">{T.stack.learningTitle}</h3>
            <div className="flex flex-wrap gap-3">
              {T.stack.learning.map((tech) => (
      ...
      <section id="proyectos" key="proyectos" className="border-t border-zinc-800/50 py-24">
        <div className="max-w-6xl mx-auto px-6">
          <ScrollReveal direction="left" className="mb-12">
            <span className="text-violet-400 font-mono text-sm">{T.projects.comment}</span>
            <h2 className="text-3xl lg:text-4xl font-bold text-white mt-2">{T.projects.heading}</h2>
            <p className="text-zinc-500 text-sm mt-2 max-w-xl">{T.projects.subtitle}</p>
          </ScrollReveal>
          <ScrollReveal direction="up">
      ...
      <section id="contacto" key="contacto" className="border-t border-zinc-800/50 bg-zinc-900/20 py-24">
        <div className="max-w-6xl mx-auto px-6">
          <ScrollReveal direction="up" className="mb-12">
            ...
            <h2 className="text-3xl lg:text-4xl font-bold text-white mt-2">{T.contact.heading}</h2>
            <p className="text-zinc-400 text-lg max-w-2xl mt-2 leading-relaxed">{T.contact.subtitle}</p>
          </ScrollReveal>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {contactCards.map((opt) => (
              <ContactCard key={opt.label} opt={opt} />
            ))}
          </div>
